...
describe Barracks do
  ...
  describe "#can_buy_seage_engine?" do
    it "returns true if there are enough resources to buy seage engine" do
      expect(@barracks.can_buy_seage_engine?).to be_truthy
    end

    it "returns false if there isn't enough food" do
     
      expect(@barracks).to receive(:food).and_return(2)
      expect(@barracks.can_buy_seage_engine?).to be_falsey
    end
  ...
  describe "#buy_seage_engine" do
    it "does not buy_seage_engine if there aren't enough resources" do
      expect(@barracks).to receive(:can_buy_seage_engine?).and_return(false)
      expect(@barracks.buy_seage_engine).to be_nil
    end
    it "buy_seage_engine if there are enough resources" do
      expect(@barracks).to receive(:can_buy_seage_engine?).and_return(true)
      expect(@barracks.buy_seage_engine).to be_a(SeigeEngine)
    end
  end
